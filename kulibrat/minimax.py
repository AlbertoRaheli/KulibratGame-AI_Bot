...
def minimax(board, depth, max_player, alpha, beta):
    ...
    if max_player==RED:
        ...
        for move in get_all_moves(board, RED):
            evaluation = minimax(move, depth-1, BLUE, alpha, beta)[0]
            max_eval = max(max_eval, evaluation)
            if max_eval == evaluation:
                best_move = move
            if winning_stack(move) or winning_row(move):
                return 1000, move
                break
        ...
        for move in get_all_moves(board, BLUE):
            evaluation = minimax(move, depth-1, RED, alpha, beta)[0]
...
def evaluate2(board):
        ...
        if board.score_red == 5:
            goal_r = 500
        if board.score_blue == 5:
            goal_b = 5000
        ...
        A_red = [[0, 0, 0],[4, 5, 4],[7, 10, 7], [11, 14, 11], [18, 18, 18], [goal_r, goal_r, goal_r]]
        A_blue = [[goal_b, goal_b, goal_b],[30, 30, 30], [16, 22, 16], [7, 10, 7], [4, 5, 4],[0, 0, 0]]
